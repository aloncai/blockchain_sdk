...
function AscSdk(provider) {
    ...
    this.minPrice = provider.testnet === true? "20000000000": "1000000000000000"
}

/**
 * 创建交易体
 */
AscSdk.prototype.createTxInfo = async function ({ from, to, value, fee, data = '0x', nonce }) {
    if ( nonce == undefined ) {
        throw '请传入nonce'
    }
    // 验证地址是否有效
    ...
    var that = this;
    var gasPrice = this.minPrice;
    // gasPrice = Bignumber(gasPrice).gt(minPrice)? gasPrice: minPrice;
    ...
    var tx = new EthereumTx({
        ...
        gasLimit: `0x${bigGasPrice.times(Bignumber(gas)).toString(16)}`,
        to,
        data,
...
AscSdk.prototype.isValidPrivate = function (privateKey) {
    if (!utils.isValidPrivate(utils.toBuffer(privateKey))) {
        throw new Error('无效的私钥')
    }
}

AscSdk.prototype.getGas = async function(sender, recipient, value, contract) {
...
AscSdk.prototype.getTokenAverageFee = async function (sender, recipient, value, contract) {
...
AscSdk.prototype.createTx = async function (privateKey, recipientAddress, amount, fee, contract, data, nonce, needValue = false) {
...
AscSdk.prototype.send = async function (txData) {
...
//查询余额
AscSdk.prototype.getBalance = async function (address, contract) {
    this.isValidAddress(address);
    return await this.web3.eth.getBalance(address)
}

AscSdk.prototype.getErc20Balance = async function(contract,address) {
    this.isValidAddress(contract);
    this.isValidAddress(address);
    var token = new this.web3.eth.Contract(abi, contract)
...
// 获取合约symbol
AscSdk.prototype.getErc20Symbol = async function(contract) {
    this.isValidAddress(contract);
    var token = new this.web3.eth.Contract(abi, contract)
...
// 获取合约内容
AscSdk.prototype.getErc20Decimal = async function(contract) {
    this.isValidAddress(contract);
    var token = new this.web3.eth.Contract(abi, contract)
...
// 是否有效的以太坊地址
AscSdk.prototype.isValidAddress = function (address) {
...
// 验证地址是否合法
AscSdk.prototype.checkAddress = function (address) {
    if (!utils.isValidAddress(address)) {
        return false;
    }
    return true;
...
//查询交易
AscSdk.prototype.findTx = async function (txHash, multiple) {
...
AscSdk.prototype.isErc20Tx = function (txData, receipt) {
...
AscSdk.prototype.toLowCase = function(str) {
    if(str) {
 ...
 */
AscSdk.prototype.getCommonTx = function (txData, receiptData, ercData) {
    // 交易状态
    var status = null;
    if (!txData.blockHash) {
        status = 'pending';
...
AscSdk.prototype.findTxByBlock = async function (numberOrHash) {
    // 存放通用交易
...
AscSdk.prototype.findBlockHeaderByBlock = async function (numberOrHash) {
        const that = this;
        return new Promise(async (resolve, reject)=>{
            try {
...
AscSdk.prototype.getLastBlock = async function () {
...
AscSdk.prototype.createKeypair = async function () {
...
AscSdk.prototype.getPublicKeyAndAddress = function (privateKey) {
...
AscSdk.prototype.getAddress = function (publicKey) {
...
AscSdk.prototype.getNonce = async function (address) {
    var nonce = await this.web3.eth.getTransactionCount(address, 'pending')
    return nonce;
}

AscSdk.prototype.getAverageFee = async function () {
    var that = this;
...
AscSdk.prototype.getPrivateKeyBySeed = function(seed) {
    // 旧生成方式
...
AscSdk.prototype.getBlock = async function (numberOrHash) {
...
AscSdk.prototype.approve = async function (privateKey, spender, amount, fee, contract, nonce) {
 ...
 */
AscSdk.prototype.allowance = async function (tokenContract, owner, spender) {
